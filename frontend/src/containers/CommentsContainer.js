...
class CommentsContainer extends Component {
  ...
  renderConfirmation = (commentId) => {
    if (this.state.renderDeleteConfirmation === commentId) {
      return (
        <h5>
          <Badge variant="light">Are you sure?</Badge>
          <button
            className="confirm-delete-button"
            onClick={() => this.handleDeleteConfirmation(commentId)}
          >
            Yes
          </button>
        </h5>
  ...
  handleDeleteClick = (commentId) => {
    this.setState({
      renderDeleteConfirmation: !!this.state.renderDeleteConfirmation
        ? null
        : commentId,
    });
  };
      ...
      return this.props.comments.map((comment) => {
        return (
          <div className="comment-box" key={comment.id}>
            {this.props.user === comment.attributes.user
              ? this.renderDeleteButton(comment.id)
              : null}
